...
class EntryGrid {
    ...
    async loadPage(page) {
        ...
        try {
            const res = await ApiClient.fetch(
                `/characters/${this.currentSource}?page=${page}&page_size=${this.pageSize}`
            );
    ...
    editCell(rowIdx, colIdx) {
        ...
        const cell = this.tableBody.querySelector(
            `tr[data-ref="${htmlEscape(item.src_ref)}"] .cell-editable[data-row="${rowIdx}"][data-col="${colIdx}"]`
        );
        if (!cell || cell.classList.contains('editing')) return;

        const currentValue = colIdx === 3 ? (item.pronunciation || '') : (item.meaning || '');
        const isMeaning = colIdx === 4;

        cell.classList.add('editing');
        cell.innerHTML = isMeaning
            ? `<textarea class="edit-textarea" rows="2">${htmlEscape(currentValue)}</textarea>`
            : `<input class="edit-input" type="text" value="${htmlEscape(currentValue)}">`;
        ...
        const doCommit = () => {
            ...
            committing = true;
            this._commitCell(rowIdx, colIdx, cell, input).finally(() => { committing = false; });
        };
    ...
    async _commitCell(rowIdx, colIdx, cell, input) {
        ...
        const formatted = applyFormat
            ? (isPron ? this._formatPronunciation(rawValue) : this._formatMeaning(rawValue))
            : rawValue;
    ...
    async jumpToRef() {
        ...
        if (!this.currentSource) { Notification.show('请先选择来源', 'error'); return; }
        ...
        try {
            ...
            const BATCH = 1000;
            while (true) {
                const batch = await ApiClient.fetch(
                    `/characters/${this.currentSource}?page=${page}&page_size=${BATCH}`
                );
