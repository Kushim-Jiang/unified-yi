...
class AlignmentApp {
    ...
    _buildSourceToggles() {
        ...
        if (!container) return;
        container.innerHTML = this.st.sources.map(s => {
    ...
    _renderPickPagination(r) {
        ...
        if (!pag) return;
        if (r.total_pages <= 1) { pag.innerHTML = ''; return; }
    ...
    pagePick(p) { this.st.pickPage = p; this._loadPickList(); }

    jumpPickPage() {
        const input = document.getElementById('pickPageInput');
        const p = parseInt(input?.value, 10);
        if (isNaN(p) || p < 1) { input.value = this.st.pickPage; return; }
        this.st.pickPage = p;
        this._loadPickList();
    }
    ...
    async addSuggestionToGroup(source, srcRef) {
        ...
        if (this.st.alignedKeys.has(key)) { Notification.show('Already in an existing group', 'info'); return; }
        if (this.st.currentGroup.some(e => `${e.source}:${e.src_ref}` === key)) return;
    ...
    async removeFromGroup(idx) {
        ...
        if (removed && removed.source === this.st.pickSource) {
            ...
            if (row) {
                ...
                row.onclick = function () { app.pickChar(ref); };
                const statusCell = row.querySelector('.status-cell');
    ...
    async _doFetchBatchSuggestions() {
        ...
        try {
            ...
            try {
                ...
                this.st.groupSearch = '';
            } catch (e) { this.st.groupSuggestions = []; this.st.groupSearch = ''; }
    ...
    _renderBatchSuggestions() {
        ...
        const sourceCards = sources.map(src => {
            ...
            return `<div style="flex:1;min-width:180px;background:var(--bg);border:1px solid var(--border);border-radius:var(--radius);padding:8px;">
                ...
                ${sr.length > 0 ? `<div style="max-height:180px;overflow-y:auto;margin-bottom:6px;border-bottom:1px solid var(--border);padding-bottom:4px;">
            ...
            const filtered = query
                ? this.st.groupSuggestions.filter(gs => {
                    ...
                    return (gs.preview || []).some(p => {
                        const c = p.char || {};
                        return (p.source || '').toLowerCase().includes(query) ||
    ...
    async searchRefInSource(src) {
        ...
        if (!query) { Notification.show('请输入搜索词', 'error'); return; }
    ...
    async addSearchResultToGroup(src, srcRef) {
        ...
        if (this.st.alignedKeys.has(key)) { Notification.show('已在其他组中', 'info'); return; }
        if (this.st.currentGroup.some(e => `${e.source}:${e.src_ref}` === key)) { Notification.show('已在当前组中', 'info'); return; }
    ...
    async saveCurrentGroup() {
        ...
        const entries = this.st.currentGroup.map(e => e.src_ref);
        const seen = new Set();
        const unique = entries.filter(ref => { if (seen.has(ref)) return false; seen.add(ref); return true; });
    ...
    async loadGroupFromExisting(groupId) {
        ...
        try {
            ...
            if (!grp) { this.progress.hide(); Notification.show('Group not found', 'error'); return; }
    ...
    async addGroupToCurrent(groupId) {
        ...
        try {
            ...
            if (added === 0) { this.progress.hide(); Notification.show('All entries already in current group', 'info'); return; }
